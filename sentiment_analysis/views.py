    ...
    if request.method == "GET":
        ...
        if int(form.data["choice"]) == 1:
            ...
            if not tw.result_text:
                ...
                return render(
                    request, "sentiment_analysis/result item not found.html", context
                )
...
def saved_tweets(request, userId):
    ...
    context = {"result_for": t.result_user_name[0], "result_all": t.savedTweetsZip}
...
def saved_keyword_tweets(request, keyword):
    ...
    context = {"result_for": k.result_keyword[0], "result_all": k.savedTweetsZip}
...
def analyse_tweets(request, analysis_item):
    ...
    graph_one_render = FusionCharts(
        "column2D", "graph_one", "600", "350", "chart_one", "json", graph_one
    ).render()
    ...
    gauge_one["colorRange"] = {
        "color": [
            ...
            {"minValue": "50", "maxValue": "100", "code": "#2ecc71"},
        ]
    }

    gauge_one["dials"] = {"dial": [{"value": (pointer_location)}]}
    ...
    gauge_two["dials"] = {"dial": [{"value": (tweets_sentiment_score)}]}
