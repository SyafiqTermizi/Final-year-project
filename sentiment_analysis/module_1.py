...
class module_one:
    ...
    def searhByUser(self, searchItem, searchCount, maxId):
        ...
        for tweet in self.api.user_timeline(
            screen_name=searchItem, count=searchCount, max_id=maxId
        ):
            if tweet.user.id == 475820825:
                ...
                self.result_in_reply_to_screen_name.append(
    ...
    def createKeywordListZip(self):
        self.result_keyword_list_zip = zip(
            self.result_retweet_count,
            self.result_geo,
            self.result_created_at,
            self.result_text,
    ...
    def createTrendDataZip(self):
        self.result_trend_data_zip = zip(
            self.result_trend_name, self.result_trend_volume
        )
    ...
    def saveKeyword(self, searchItem, searchCount):
        ...
        while counter < searchCount:
            ...
            if word_search.keyword_tweets_set.filter(
    ...
    def displaySavedItem(self):
        ...
        self.result_keyword_list_zip = zip(
            self.result_keyword, self.result_search_count
        )
